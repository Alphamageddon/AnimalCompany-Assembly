namespace UnityEngine.Experimental.Rendering.RenderGraphModule;

public class RenderGraphDefaultResources
{
	private bool m_IsValid; //Field offset: 0x10
	private RTHandle m_BlackTexture2D; //Field offset: 0x18
	private RTHandle m_WhiteTexture2D; //Field offset: 0x20
	private RTHandle m_ShadowTexture2D; //Field offset: 0x28
	[CompilerGenerated]
	private TextureHandle <blackTexture>k__BackingField; //Field offset: 0x30
	[CompilerGenerated]
	private TextureHandle <whiteTexture>k__BackingField; //Field offset: 0x38
	[CompilerGenerated]
	private TextureHandle <clearTextureXR>k__BackingField; //Field offset: 0x40
	[CompilerGenerated]
	private TextureHandle <magentaTextureXR>k__BackingField; //Field offset: 0x48
	[CompilerGenerated]
	private TextureHandle <blackTextureXR>k__BackingField; //Field offset: 0x50
	[CompilerGenerated]
	private TextureHandle <blackTextureArrayXR>k__BackingField; //Field offset: 0x58
	[CompilerGenerated]
	private TextureHandle <blackUIntTextureXR>k__BackingField; //Field offset: 0x60
	[CompilerGenerated]
	private TextureHandle <blackTexture3DXR>k__BackingField; //Field offset: 0x68
	[CompilerGenerated]
	private TextureHandle <whiteTextureXR>k__BackingField; //Field offset: 0x70
	[CompilerGenerated]
	private TextureHandle <defaultShadowTexture>k__BackingField; //Field offset: 0x78

	public private TextureHandle blackTexture
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle blackTexture3DXR
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle blackTextureArrayXR
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle blackTextureXR
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle blackUIntTextureXR
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle clearTextureXR
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle defaultShadowTexture
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle magentaTextureXR
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle whiteTexture
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	public private TextureHandle whiteTextureXR
	{
		[CompilerGenerated]
		 get { } //Length: 8
		[CompilerGenerated]
		private set { } //Length: 8
	}

	internal RenderGraphDefaultResources() { }

	internal void Cleanup() { }

	[CompilerGenerated]
	public TextureHandle get_blackTexture() { }

	[CompilerGenerated]
	public TextureHandle get_blackTexture3DXR() { }

	[CompilerGenerated]
	public TextureHandle get_blackTextureArrayXR() { }

	[CompilerGenerated]
	public TextureHandle get_blackTextureXR() { }

	[CompilerGenerated]
	public TextureHandle get_blackUIntTextureXR() { }

	[CompilerGenerated]
	public TextureHandle get_clearTextureXR() { }

	[CompilerGenerated]
	public TextureHandle get_defaultShadowTexture() { }

	[CompilerGenerated]
	public TextureHandle get_magentaTextureXR() { }

	[CompilerGenerated]
	public TextureHandle get_whiteTexture() { }

	[CompilerGenerated]
	public TextureHandle get_whiteTextureXR() { }

	internal void InitializeForRendering(RenderGraph renderGraph) { }

	[CompilerGenerated]
	private void set_blackTexture(TextureHandle value) { }

	[CompilerGenerated]
	private void set_blackTexture3DXR(TextureHandle value) { }

	[CompilerGenerated]
	private void set_blackTextureArrayXR(TextureHandle value) { }

	[CompilerGenerated]
	private void set_blackTextureXR(TextureHandle value) { }

	[CompilerGenerated]
	private void set_blackUIntTextureXR(TextureHandle value) { }

	[CompilerGenerated]
	private void set_clearTextureXR(TextureHandle value) { }

	[CompilerGenerated]
	private void set_defaultShadowTexture(TextureHandle value) { }

	[CompilerGenerated]
	private void set_magentaTextureXR(TextureHandle value) { }

	[CompilerGenerated]
	private void set_whiteTexture(TextureHandle value) { }

	[CompilerGenerated]
	private void set_whiteTextureXR(TextureHandle value) { }

}

